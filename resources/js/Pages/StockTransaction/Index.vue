                    ...
                    Transaction Date
                        ...
                        <tbody class="text-[13px] font-normal">
                            <tr v-for="(stock, index) in allStockTransactions" :key="stock.id"
                                ...
                                <td class="p-4 font-bold border-gray-200">
                                    {{ stock.quantity != null ? formatQuantity(stock.quantity, stock.product?.type) : "N/A" }}
                                </td>
...
const formatQuantity = (value, productType = null) => {
    const num = Number(value);
    if (Number.isNaN(num)) {
        return value;
    }
    const quantity = Number(num.toFixed(3)).toString();
    return productType === "weight_based" ? `${quantity} kg` : quantity;
};
